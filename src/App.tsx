...
function Pill({ on, click, children }: { on: boolean; click: () => void; children: ReactNode }) {
  return <button onClick={click} className={`px-6 py-2 rounded-full text-xs font-black transition ${on ? 'bg-white text-black shadow-lg' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}>{children}</button>;
}
...
export default function App() {
  ...
  const goNext = useCallback((auto: boolean) => {
    ...
    player.loadVideo(q[n].videoId); setPlaying(true);
    setTimeout(() => player.play(), 300);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const onState = useCallback((state: number) => {
    if (state === 0) (goNext as any)(true);
    else if (state === 1) {
      setPlaying(true);
      setTimeout(() => {
  ...
  const player = useYouTubePlayer('yt-player-hidden', onState, () => (goNext as any)(false));
  ...
  const goPrev = useCallback(() => {
    ...
    player.loadVideo(queue[n].videoId); setPlaying(true);
    setTimeout(() => player.play(), 300);
  }, [player, queue, idx]);
  ...
  const playSong = (v: YTVideo) => {
    if (!player.isReady) return;
    setTime(0); setDur(v.duration || 0);
    const ei = queue.findIndex(q => q.videoId === v.videoId);
    if (ei >= 0) setIdx(ei); else { setQueue(prev => [...prev, v]); setIdx(queue.length); }
    player.loadVideo(v.videoId); setPlaying(true);
  ...
  const playAll = (vids: YTVideo[], start = 0) => {
    ...
    player.loadVideo(vids[start].videoId); setPlaying(true);
    setTimeout(() => player.play(), 400);
  ...
  return (
    <div className="flex h-screen bg-[#121212] text-white font-sans select-none overflow-hidden relative">
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 bg-[#181818] border-t border-white/5 shadow-2xl pb-safe"><div className="w-full h-1 bg-white/5 cursor-pointer" onClick={e => { const r = e.currentTarget.getBoundingClientRect(); player.seekTo(((e.clientX - r.left) / r.width) * dur); }}><div className="h-full bg-red-600 transition-all" style={{ width: `${pct}%` }} /></div><div className="max-w-[2000px] mx-auto flex items-center justify-between px-4 py-3 gap-4"><div className="flex items-center gap-3 w-1/3 min-w-0">{currentTrack ? <><div className="w-14 h-14 rounded-lg overflow-hidden relative flex-shrink-0">{currentTrack.thumbnail ? <img src={currentTrack.thumbnail} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full" style={{ background: grad(currentTrack.videoId) }} />}{playing && <div className="absolute inset-0 bg-black/40 flex items-center justify-center"><div className="flex items-end gap-0.5 h-4"><span className="w-[2.5px] bg-red-500 rounded-full h-[40%] animate-pulse" /><span className="w-[2.5px] bg-red-500 rounded-full h-[90%] animate-pulse" /><span className="w-[2.5px] bg-red-500 rounded-full h-[60%] animate-pulse" /></div></div>}</div><div className="min-w-0"><div className="text-sm font-bold truncate">{currentTrack.title}</div><div className="text-xs text-white/50 truncate">{currentTrack.channelTitle}</div></div><button onClick={() => handleLike(currentTrack)} className="flex-shrink-0 ml-2 text-white/40 hover:text-white transition"><Hart on={likedSet.has(currentTrack.videoId)} sz={20} /></button></> : <span className="text-sm text-white/20 font-bold uppercase tracking-widest italic">No music selected</span>}</div><div className="flex flex-col items-center gap-1.5 flex-1 max-w-lg"><div className="flex items-center gap-6"><button onClick={() => setShuf(!shuf)} className={shuf ? 'text-red-500' : 'text-white/40 hover:text-white'}><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M10.59 9.17L5.41 4 4 5.41l5.17 5.17 1.42-1.41zM14.5 4l2.04 2.04L4 18.59 5.41 20 17.96 7.46 20 9.5V4h-5.5zm.33 9.41l-1.41 1.41 3.13 3.13L14.5 20H20v-5.5l-2.04 2.04-3.13-3.13z" /></svg></button><button onClick={goPrev} className="text-white/80 hover:text-white transition"><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg></button><button onClick={() => playing ? player.pause() : player.play()} disabled={!player.isReady || !currentTrack} className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center shadow-lg">{playing ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z" /></svg> : <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>}</button><button onClick={() => (goNext as any)(false)} className="text-white/80 hover:text-white transition"><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6h2v12h-2z" /></svg></button><button onClick={() => setRep(r => r === 'off' ? 'all' : r === 'all' ? 'one' : 'off')} className={`relative ${rep !== 'off' ? 'text-red-500' : 'text-white/40 hover:text-white'}`}><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z" /></svg>{rep === 'one' && <span className="absolute -top-1 -right-1 text-[8px] font-bold">1</span>}</button></div><div className="flex items-center gap-2 w-full text-[10px] font-mono text-white/40"><span className="w-10 text-right">{fmtTime(time)}</span><div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-white/60" style={{ width: `${pct}%` }} /></div><span className="w-10">{fmtTime(dur)}</span></div></div><div className="flex items-center justify-end gap-3 w-1/3"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-white/40"><path d="M7 9v6h4l5 5V4l-5 5H7zm9.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z" /></svg><div className="w-24 h-1 bg-white/5 rounded-full cursor-pointer relative" onClick={e => { const r = e.currentTarget.getBoundingClientRect(); setVol(((e.clientX - r.left) / r.width) * 100); }}><div className="h-full bg-white/40" style={{ width: `${vol}%` }} /></div></div></div></footer>
